...
levelOf square = length $ takeWhile (\a -> a < square) $ fmap (\i -> totalSquares i) [0,1..]
...
sumValue_opt _ 1 = 1
sumValue_opt previous square = sum $ fmap (\s -> snd $ fromJust $ find (\(a,_) -> a == s) previous) others
  where level = levelOf square
        others = filter (isNeighbouring square) $ takeWhile (\x -> levelOf x >= level - 1) $ reverse [1..square-1]

sumValues_opt = reverse $ fmap snd $ foldr (\x xs -> (x,sumValue_opt xs x) : xs ) [] $ reverse [1..2000]

solve1 = distance
solve2 inp = head $ dropWhile (<= inp) sumValues_opt
